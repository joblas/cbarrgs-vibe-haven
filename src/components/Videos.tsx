...
const Videos: React.FC = () => {
  ...
  return <section id="videos" className="relative py-24 md:py-32 bg-zinc-900">
      <div className="section-container">
        ...
        <motion.div initial="initial" animate={controls} variants={slideUp(0.3)} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {VIDEOS.map((video, index) => <VideoCard key={video.id} title={video.title} thumbnail={video.thumbnail} embedUrl={video.embedUrl} index={index} />)}
        </motion.div>
